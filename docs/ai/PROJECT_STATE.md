...
| 2F | explicit create и immutable ownership постоянного PERSONAL_COMPANION вне slots owner | [ADR-0006](ADR-0006-PRODUCT-COMPANION-CREATION.md) | этот commit |
...
- Один shared deactivated host account допустим для текущего starter state, но
  перед economy/inventory progression нужен отдельный аудит account-scoped state.
