...
Зависимость: явное подтверждение
[ADR-0001](ADR-0001-SERVER-CONTROLLED-PLAYER.md) владельцем
...
префиксом `[AI]`. Два настоящих клиента видят его как игрока. Он циклически
проходит 3–5 близких точек, передаёт координаты, heading и анимацию, корректно
появляется для позднего наблюдателя и удаляется у всех.
...
## 2. Предварительно подтверждаемые решения

До кода необходимо подтвердить:
...
3. Находит configured template character и вызывает
   `PlayerService.getPlayer(characterId, account)`.
...
## 7. Ручная проверка двумя клиентами
...
После выполнения этих критериев агент останавливается. Бой, квесты,
компаньон, экономика и массовый spawn начинаются только после отдельного
подтверждения.
